...
const initial = {},
	...
	rangechange = action('RANGECHANGE', (first, last) => ({ range: [first, last]})),
		...
		return {
			...
			rangechange: useCallback(
				({
					...
					if (range != null && range[0] === first && range[1] === last) {
						return;
					}
					dispatch(rangechange(first, last));
				},
				[dispatch, range]
			)
		};
	...
	useSizer = ({
		...
		const [sizer, setSizer] = useState(() => textual(items[0]));

		useEffect(() => {
			if (range == null) {
				return;
			}
			const newSizer = items.slice(...range).reduce((p, item) => {
				const text = textual(item);
				return text.length > p.length ? text : p;
			}, sizer);
			newSizer !== sizer && setSizer(newSizer);
		}, [items, range, textual]);
