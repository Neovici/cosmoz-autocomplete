...
const chip = (text, onClear, disabled) => html`
		...
		return html`
			...
			${ !isSingle && items.length && portal(html`
				<cosmoz-suggestions
					...
					.placement=${ placement }
					.defaultIndex=${ isNaN(Number(defaultIndex)) ? undefined : Number(defaultIndex) }
				/>`) || nothing }
		`;
	},
	...
	observedAttributes = [...inputAttributes.slice(3), 'text-property', 'limit', 'hide-empty', 'default-index'];
