...
import style, { listboxStyle } from './autocomplete.css';

const
	blank = () => nothing,
	renderChip = (text, onClear, disabled) => html`
		<div class="chip" part="chip" slot="suffix" title=${ text }>
	...
	renderSelection = (value, isOne, onDeselect, textual, disabled) => {
		if (isOne) {
			const val = value[0];
			return val ? renderChip(textual(val), () => onDeselect(val), disabled) : nothing;
		}
	...
	autocomplete = props => {
		const
			...
			suggestions = until(items$.then(items => (!isSingle || showSingle) && items.length && listbox({
				...
				anchor,
				items,
				styles: !isOne ? listboxStyle : undefined
			}) || nothing));
