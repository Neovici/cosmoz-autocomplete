import{T as B,f as Ie,B as Cn,E as pt,m as Fe,x as E,i as En,p as Sn,v as J,r as ht,M as jt,_ as zn}from"./iframe-Dw6xcovl.js";/**
 * @license
...
`,fs=e=>typeof e=="object"&&e!==null&&Symbol.iterator in e;function ot(e){return e==null?[]:Array.isArray(e)?e:typeof e=="string"?[e]:fs(e)?Array.from(e):[e]}const qt=(e,t=Tt)=>n=>{const i=ot(e).map(t);return ot(n).filter(s=>!i.includes(t(s)))},ps=(e,t)=>t?n=>ot(e).find(i=>i[t]===n[t]):n=>ot(e).includes(n),ms=(e,t)=>{if(!t||!e)return e;const n=e.toLowerCase().indexOf(t.toLowerCase());if(n<0)return e;const i=n+t.length;return[e.slice(0,n),E`<mark>${e.slice(n,i)}</mark>`,e.slice(i)]},_t=e=>{const t=R(()=>({}),[]);return R(()=>Object.assign(t,e),[t,...Object.values(e)])},gs=e=>{const t=_t(e);S(()=>{const n=i=>{if(!(i.ctrlKey&&i.altKey||i.defaultPrevented))switch(i.key){case"Up":case"ArrowUp":i.preventDefault(),t.onUp();break;case"Down":case"ArrowDown":i.preventDefault(),t.onDown();break;case"Enter":i.preventDefault(),t.onEnter();break}};return document.addEventListener("keydown",n,!0),()=>document.removeEventListener("keydown",n,!0)},[t])},bs=({items:e,onSelect:t,defaultIndex:n=0})=>{const[i,s]=X({index:n}),{index:o}=i,{length:r}=e;return S(()=>{s({index:i.index<0?n:Math.min(i.index,e.length-1),scroll:!0})},[e,n]),gs({onUp:x(()=>s(l=>({index:l.index>0?l.index-1:r-1,scroll:!0})),[r]),onDown:x(()=>s(l=>({index:l.index<r-1?l.index+1:0,scroll:!0})),[r]),onEnter:x(()=>o>-1&&o<r&&t?.(e[o],o),[o,e,t])}),{position:i,highlight:x(l=>s({index:l}),[]),select:x(l=>t?.(l),[t])}},vs=(e=Tt)=>(t,n,{highlight:i,select:s,textual:o=Tt,query:r,isSelected:l})=>{const a=o(t),c=ms(a,r),h=e(c,t,n);return E`<div
			...
			<div class="sizer" virtualizer-sizer>${h}</div>`},_s=({itemRenderer:e=vs(),...t})=>{const n=_t(t);return x((i,s)=>e(i,s,n),[n,e])},ys=e=>{const t=e==="auto",[n,i]=X(t?40:e);return[n,s=>t?i(s):void 0]},ws=["query","items","onSelect","textual","anchor","itemHeight","itemLimit","itemRenderer","defaultIndex","value","valueProperty","loading"],xs=({value:e,valueProperty:t,items:n,onSelect:i,defaultIndex:s,query:o,textual:r,itemRenderer:l,itemHeight:a=40,itemLimit:c=5})=>{const h=R(()=>ps(e,t),[e,t]),u=R(()=>n.slice(),[n,h]),{position:f,highlight:d,select:b}=bs({items:u,onSelect:i,defaultIndex:isNaN(s)?void 0:Number(s)}),[p,g]=ys(a);return{position:f,items:u,height:Math.min(c,u.length)*p,highlight:d,select:b,itemHeight:p,setItemHeight:g,renderItem:_s({itemRenderer:l,items:u,position:f,highlight:d,select:b,textual:r,query:o,isSelected:h})}},Te=ls,Cs=e=>{const t=je(void 0),{position:n,items:i,renderItem:s,height:o,itemHeight:r,setItemHeight:l}=xs(e);return S(()=>{const a=t.current?.[Qt];a&&a.layoutComplete.then(()=>{e.dispatchEvent(new CustomEvent("layout-complete"));const{averageChildSize:c,averageMarginSize:h}=a._layout._metricsCache;return l(c+h*2)},Te)},[i]),S(()=>{if(!n.scroll)return;const a=t.current?.[Qt];if(a){if(!a?._layout){a.layoutComplete.then(()=>a.element(n.index)?.scrollIntoView({block:"nearest"}),Te);return}a.element(n.index)?.scrollIntoView({block:"nearest"})}},[n]),ss(ds({...n,itemHeight:r,auto:e.itemHeight==="auto"})),E`<div
			class="items"
